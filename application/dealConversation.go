...
func DealConversation_HandlerSave(w http.ResponseWriter, r *http.Request) {
	...
	item.ContractNumber = r.FormValue("ContractNumber")
	item.AckReference = r.FormValue("AckReference")
	item.NewTX = r.FormValue("NewTX")
	item.LegNo = r.FormValue("LegNo")
	item.Summary = r.FormValue("Summary")
	item.BusinessDate = r.FormValue("BusinessDate")
	item.TXNo = r.FormValue("TXNo")
	...
	// adaptor.DealConversation_XMLExport(item)
