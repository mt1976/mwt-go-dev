...
CREATE VIEW [{{!SQL.SCHEMA}}].[CryptoCurrency]
AS
...
WHERE        ({{!SQL.SOURCE}}.CurrencyPairRateRICCode.RICCode = 'SRS=CRYPTO') AND ({{!SQL.SOURCE}}.CurrencyPair.SpotRerouteDataRerouteCurrency = '') AND ({{!SQL.SOURCE}}.CurrencyPairRateRICCode.Period = 'Spot') AND ({{!SQL.SOURCE}}.CurrencyPair.Active = 1)
