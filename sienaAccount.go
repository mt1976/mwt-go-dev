...
func getSienaAccountList(db *sql.DB) (int, []sienaAccountItem, error) {
	...
	tsql := fmt.Sprintf("SELECT SienaReference, CustomerSienaView, Status,StartDate,ExternalReference,CCY,Book,MandatedUser,BackOfficeNotes,CashBalance,AccountNumber,AccountName,LedgerBalance,Portfolio,UTI,CCYName,BookName FROM %s.sienaAccount;", mssqlConfig["schema"])
	...
	count := 0
	for rows.Next() {
		var SienaReference, Counterparty, Status, CCY, Book, AccountNumber, AccountName, CCYName, BookName string
		...
		err := rows.Scan(&SienaReference, &Counterparty, &Status, &Opened, &ExternalReference, &CCY, &Book, &MandatedUser, &BackOfficeNotes, &CashBalance, &AccountNumber, &AccountName, &LedgerBalance, &Portfolio, &UTI, &CCYName, &BookName)
		if err != nil {
			log.Println("Error reading rows: " + err.Error())
			return -1, nil, err
		}
...
func getSienaAccount(db *sql.DB, id string) (int, sienaAccountItem, error) {
	...
	tsql := fmt.Sprintf("SELECT SienaReference, CustomerSienaView, Status,StartDate,ExternalReference,CCY,Book,MandatedUser,BackOfficeNotes,CashBalance,AccountNumber,AccountName,LedgerBalance,Portfolio,UTI,CCYName,BookName FROM %s.sienaAccount WHERE SienaReference='%s';", mssqlConfig["schema"], id)
	fmt.Println("MS SQL:", tsql)
