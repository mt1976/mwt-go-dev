...
func MarketRates_GetList() (int, []dm.MarketRates, error) {
	...
	tsql := "SELECT * FROM " + get_TableName(core.ApplicationSQLSchema(), dm.MarketRates_SQLTable)
...
func MarketRates_Delete(id string) {
	...
	object_Table := core.ApplicationSQLSchema() + "." + dm.MarketRates_SQLTable
...
func marketrates_Save(r dm.MarketRates, usr string) error {
	...
	tsql := "INSERT INTO " + get_TableName(core.ApplicationSQLSchema(), dm.MarketRates_SQLTable)
	tsql = tsql + " (" + fields(ts) + ")"
